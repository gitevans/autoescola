			...
			$query_carga_t = "SELECT SUM(hora) FROM carga_legislacao";
			$carga_t = mysql_query($query_carga_t, $conexao) or die(mysql_error());
			$row_carga_t = mysql_fetch_assoc($carga_t);
			$totalRows_carga_t = mysql_num_rows($carga_t);
			
			mysql_select_db($database_conexao, $conexao);
			$query_soma = "select SUM(qtd) from carga_legislacao";
			$soma = mysql_query($query_soma, $conexao) or die(mysql_error());
			$row_soma = mysql_fetch_assoc($soma);
			$totalRows_soma = mysql_num_rows($soma);
			
			$hora2 = $row_soma['SUM(qtd)'];
			
			$hora = $row_carga_t['SUM(hora)'];
...
<div class="container-fluid">
	<div class="row-fluid"> 
			...
			<form name="form2" method="post" action="adicionar_carga_teoricas.php">
			<label>
			...
			$sql = "SELECT DISTINCT id,aluno,qtd,hora from carga order by id DESC";
			...
			<td valign="top"  class="td"><font color="#666666"><?php if ($hora2 == '')
					{ echo "<font color=\"red\">Nenhuma aula registrada !";
					} else {
					echo $hora2." Aula(s) Registrada(s) !";
					}
					?></td>
			...
			if ($horas == 5){
			echo  " <font color=\"orange\">  Hóras Apenas ! (Atenção)</font>";
			}else if($horas == 1){
			echo  "<font color=\"red\"> Carga horária esgotada !</font>";
			}else{
			echo " h&oacute;ras para conclus&atilde;o do curso";
			}
			?> 
			
			<span class="style2"></span></td>
			<td align="left" valign="top"  class="td style1"><div align="center"><a href="#" onclick="MM_openBrWindow('carga_registro.php?id=<?php echo $resultado['id']; ?>','','scrollbars=yes,resizable=yes,width=300,height=150')">Registrar</a></div></td>
			</tr>
			<tr><?php $cont ++; }?>
			<?php
			...
			echo "<a class=\"pag\" href='adicionar_carga_teoricas.php?p=1' target='_self'>&laquo; Anterior</a> ";
			...
			echo "<a class=\"pag\" href='adicionar_carga_teoricas.php?p=".$i."' target='_self'>".$i."</a> ";
			}
			}
			...
			echo "<a class=\"pag\" href='adicionar_carga_teoricas.php?p= " .$pags."' target='_self'>Pr&oacute;xima &raquo;</a> ";
